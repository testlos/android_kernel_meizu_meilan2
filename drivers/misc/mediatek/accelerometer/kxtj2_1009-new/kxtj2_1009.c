...
#else
static struct i2c_board_info i2c_kxtj2_1009 __initdata = {
	I2C_BOARD_INFO(KXTJ2_1009_DEV_NAME, (KXTJ2_1009_I2C_SLAVE_ADDR>>1))
};
#endif

/*----------------------------------------------------------------------------*/
static int kxtj2_1009_i2c_probe(struct i2c_client *client,
		const struct i2c_device_id *id);
static int kxtj2_1009_i2c_remove(struct i2c_client *client);
/* static int kxtj2_1009_i2c_detect(struct i2c_client *client, int kind,
...
static struct acc_hw accel_cust_hw;
static struct acc_hw *hw = &accel_cust_hw;
...
static struct meizu_classdev *gsensor;
#if 0
static int acc_enable_value;
#endif
static int acc_calibration_value;
static int acc_x_calibration_value;
static int acc_y_calibration_value;
static int acc_z_calibration_value;
static int acc_x_offset_value;
static int acc_y_offset_value;
static int acc_z_offset_value;
static int acc_x_cali_value;
static int acc_y_cali_value;
static int acc_z_cali_value;
#endif
...
/*---------------------------------------------------------------------------*/
static int KXTJ2_1009_SetPowerMode(struct i2c_client *client, int enable);
...
static int KXTJ2_1009_SetDataResolution(struct kxtj2_1009_i2c_data *obj)
{
	...
	if (hwmsen_read_block(obj->client, KXTJ2_1009_REG_DATA_RESOLUTION,
			databuf, 0x01)) {
...
/*----------------------------------------------------------------------------*/
static int KXTJ2_1009_ReadData(struct i2c_client *client,
		s16 data[KXTJ2_1009_AXES_NUM])
{
	...
	if (NULL == client)
		...
		if (err != 0)
			...
			for (i = 0; i < 3; i++) {
				/* because the data is store in binary complement
					number formation in computer system */
				if (data[i] == 0x0800)    /* so we want to calculate actual number here */
					...
					data[i] -= 0x1;             /* printk("data 0 step %x\n",data[i]); */
					data[i] = ~data[i];         /* printk("data 1 step %x\n",data[i]); */
					data[i] &= 0x07ff;          /* printk("data 2 step %x\n",data[i]); */
					data[i] = -data[i];         /* printk("data 3 step %x\n\n",data[i]); */
				}
			}
			...
			if (atomic_read(&priv->trace) & ADX_TRC_RAWDATA)
				GSE_LOG("[%08X %08X %08X] => [%5d %5d %5d]\n",
					...
					data[KXTJ2_1009_AXIS_Z], data[KXTJ2_1009_AXIS_X],
					data[KXTJ2_1009_AXIS_Y], data[KXTJ2_1009_AXIS_Z]);
#ifdef CONFIG_KXTJ2_1009_LOWPASS
			if ((atomic_read(&priv->filter)) &&
				atomic_read(&priv->fir_en) && !atomic_read(&priv->suspend)) {
...
/*----------------------------------------------------------------------------*/
static int KXTJ2_1009_ReadOffset(struct i2c_client *client,
		s8 ofs[KXTJ2_1009_AXES_NUM])
{
	int err = 0;
...
/*----------------------------------------------------------------------------*/
static int KXTJ2_1009_ReadCalibration(struct i2c_client *client,
...
/*----------------------------------------------------------------------------*/
static int KXTJ2_1009_ReadCalibrationEx(struct i2c_client *client,
		int act[KXTJ2_1009_AXES_NUM], int raw[KXTJ2_1009_AXES_NUM])
{
...
/*----------------------------------------------------------------------------*/
static int KXTJ2_1009_WriteCalibration(struct i2c_client *client,
		int dat[KXTJ2_1009_AXES_NUM])
{
	struct kxtj2_1009_i2c_data *obj = i2c_get_clientdata(client);
	int err;
...
static int kxtj2_1009_init_client(struct i2c_client *client, int reset_cali)
{
	...
	pr_info("kxtj2_1009_init_client OK!\n");
#ifdef CONFIG_KXTJ2_1009_LOWPASS
...
/*----------------------------------------------------------------------------*/
static int KXTJ2_1009_ReadSensorData(struct i2c_client *client,
		char *buf, int bufsize)
{
	struct kxtj2_1009_i2c_data *obj =
			(struct kxtj2_1009_i2c_data *)i2c_get_clientdata(client);
	u8 databuf[20];
...
#ifdef DEBUG_MSG_CAL
	/* pr_info("---KXTJ2 Calibration Raw Data,%d,%d,%d==> Z+=%d  Z-=%d\n",raw[0],raw[1],raw[2],Z_AVG[0],Z_AVG[1]); */
...
/*dixiaobing@wind-mobi.com 20150617 start*/
	...
	/* pr_info("dixiaobinggsensor333	acc[KXTJ2_1009_AXIS_X]=%d,acc[KXTJ2_1009_AXIS_Y]=%d,acc[KXTJ2_1009_AXIS_Z]=%d\n",acc[KXTJ2_1009_AXIS_X],acc[KXTJ2_1009_AXIS_Y],acc[KXTJ2_1009_AXIS_Z]); */
#endif
/*dixiaobing@wind-mobi.com 20150617 end*/
...
/*----------------------------------------------------------------------------*/
#if 0
static int KXTJ2_1009_JudgeTestResult(struct i2c_client *client,
		s32 prv[KXTJ2_1009_AXES_NUM], s32 nxt[KXTJ2_1009_AXES_NUM])
{
	int res = 0;
	u8 test_result = 0;
...
static ssize_t store_register(struct device_driver *ddri,
		const char *buf, size_t count)
{
...
static ssize_t store_register_value(struct device_driver *ddri,
		const char *buf, size_t count)
{
	struct kxtj2_1009_i2c_data *obj = obj_i2c_data;
	u8 databuf[2];
...
/*----------------------------------------------------------------------------*/
/*static int kxtj2_1009_ioctl(struct inode *inode, struct file *file,
		unsigned int cmd, unsigned long arg) */

static long kxtj2_1009_unlocked_ioctl(struct file *file,
		unsigned int cmd, unsigned long arg)
{
...
/*----------------------------------------------------------------------------*/
#else /* CONFIG_HAS_EARLYSUSPEND || USE_EARLY_SUSPEND */
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
#endif /* CONFIG_HAS_EARLYSUSPEND || USE_EARLY_SUSPEND */
/*----------------------------------------------------------------------------*/
		...
		const struct i2c_device_id *id)
{
	...
	pr_info("%s, acc_register_data_path\n", __func__);
...
static int __init kxtj2_1009_init(void)
{
	pr_err("kxtj2_1009_init\n");
#ifdef CONFIG_OF
	hw = get_accel_dts_func(accel_of_match[0].compatible, &accel_cust_hw);
#else
	hw = kxtj2_1009_get_cust_acc_hw(); /* wenggaojian@wind-mobi.com */
#endif
...
static DEVICE_ATTR(acc_x_calibbias, 0444, meizu_acc_x_calibbias_show, NULL);
static DEVICE_ATTR(acc_y_calibbias, 0444, meizu_acc_y_calibbias_show, NULL);
static DEVICE_ATTR(acc_z_calibbias, 0444, meizu_acc_z_calibbias_show, NULL);
