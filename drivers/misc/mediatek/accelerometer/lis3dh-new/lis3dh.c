...
#else
static struct i2c_board_info i2c_LIS3DH __initdata = { I2C_BOARD_INFO("LIS3DH", (0x30>>1))};
#endif

/*----------------------------------------------------------------------------*/
static int LIS3DH_ReadSensorData_mag(struct i2c_client *client,
...
static struct data_resolution lis3dh_data_resolution[] = {
	...
	{{ 1, 9}, 512},
	/* dataformat +/-8g  in 12-bit resolution;  { 1, 0} = 1.0 = (2*8*1000)/(2^12);  256 = (2^12)/(2*8) */
	{{ 3, 9}, 256},
};
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
/*
static int LIS3DH_ReadCalibrationEx(struct i2c_client *client,
		int act[LIS3DH_AXES_NUM], int raw[LIS3DH_AXES_NUM])
{
...
static int lis3dh_i2c_remove(struct i2c_client *client)
{
	...
	err = misc_deregister(&lis3dh_device);
	if (err)
