...
#define CAM_CALDB(fmt, arg...)     pr_debug(PFX "[%s] " fmt, __func__, ##arg)
...
#if 1
	err |= get_user(p, (u8 *)&data->pu1Params);
...
#endif
{
	...
	if (_IOC_NONE != _IOC_DIR(a_u4Command)) {
		...
		if (_IOC_WRITE & _IOC_DIR(a_u4Command)) {
			if (copy_from_user((u8 *) pBuff , (u8 *) a_u4Param, sizeof(stCAM_CAL_INFO_STRUCT))) {
				...
				kfree(pBuff);
				CAM_CALERR("[S5K3L2_OTP] ioctl copy from user failed\n");
				return -EFAULT;
			}
		}
	}
	...
	case CAM_CALIOC_S_WRITE:
		CAM_CALDB("[S5K3L2_OTP] Write CMD\n");
#ifdef CAM_CALGETDLT_DEBUG
	...
	case CAM_CALIOC_G_READ:
		...
		CAM_CALDB("[S5K3L2_OTP] After read Working buffer data  0x%x\n", *pu1Params);
	...
	if (_IOC_READ & _IOC_DIR(a_u4Command)) {
		...
		CAM_CALDB("[S5K3L2_OTP] to user length %d\n", ptempbuf->u4Length);
		CAM_CALDB("[S5K3L2_OTP] to user  Working buffer address 0x%p\n", pu1Params);
		if (copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pu1Params , ptempbuf->u4Length)) {
			kfree(pBuff);
			kfree(pu1Params);
			CAM_CALERR("[S5K3L2_OTP] ioctl copy to user failed\n");
			return -EFAULT;
		}
	}
...
{
	CAM_CALDB("[S5K3L2_OTP] CAM_CAL_Open\n");
	spin_lock(&g_CAM_CALLock);
	if (g_u4Opened) {
		spin_unlock(&g_CAM_CALLock);
		CAM_CALERR("[S5K3L2_OTP] Opened, return -EBUSY\n");
		return -EBUSY;
	}
	g_u4Opened = 1;
...
static inline int RegisterCAM_CALCharDrv(void)
{
	...
	if (alloc_chrdev_region(&g_CAM_CALdevno, 0, 1, CAM_CAL_DRVNAME)) {
		CAM_CALERR("[S5K3L2_OTP] Allocate device no failed\n");

		return -EAGAIN;
	}
#else
	if (register_chrdev_region(  g_CAM_CALdevno , 1 , CAM_CAL_DRVNAME)) {
		CAM_CALERR("[S5K3L2_OTP] Register device no failed\n");
	...
	if (NULL == g_pCAM_CAL_CharDrv) {
		...
		CAM_CALERR("[S5K3L2_OTP] Allocate mem for kobject failed\n");
	...
	if (cdev_add(g_pCAM_CAL_CharDrv, g_CAM_CALdevno, 1)) {
		CAM_CALERR("[S5K3L2_OTP] Attatch file operation failed\n");
...
static struct i2c_driver CAM_CAL_i2c_driver = {
	...
	.driver = {
		...
		.owner = THIS_MODULE,
		.of_match_table = of_match_ptr(s5k3l2_otp_dt_match),
	},
...
static int CAM_CAL_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	CAM_CALDB("[S5K3L2_OTP] Attach I2C\n");
	pr_info("--->>> enter %s ******\n", __func__);
	/* get sensor i2c client */
	spin_lock(&g_CAM_CALLock); /* for SMP */
	g_pstI2Cclient = client;
	g_pstI2Cclient->addr = S5K3L2OTP_DEVICE_ID>>1;
	spin_unlock(&g_CAM_CALLock); /* for SMP */
	CAM_CALDB("[S5K3L2_OTP] g_pstI2Cclient->addr = 0x%x\n", g_pstI2Cclient->addr);

	/* Register char driver */
	ret = RegisterCAM_CALCharDrv();

	if (ret) {
		CAM_CALDB("[S5K3L2_OTP] register char device failed!\n");
		return i4RetValue;
	}


	CAM_CALDB("[S24CAM_CAL] Attached!!\n");
	pr_info("--->>>[3M2_CAM_CAL] Attached!!\n");
	return 0;
}
...
static int __init CAM_CAL_init(void)
{
	...
	if (platform_driver_register(&g_stCAM_CAL_Driver)) {
		CAM_CALERR("failed to register s5k3l2_otp driver\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_stCAM_CAL_Device)) {
		CAM_CALERR("failed to register s5k3l2_otp driver, 2nd time\n");
		return -ENODEV;
	}
